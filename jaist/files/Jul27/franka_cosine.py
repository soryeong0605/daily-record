...
class FrankaController:
    def __init__(
        ...
        # Latest velocity command actually written to the robot (post-
        # smoother), [vx, vy, vz, 0, 0, wz]. Exposed so a data logger can
        # record the real action instead of the raw pre-smoother target --
        # avoids a state/action timing mismatch from the smoother's delay.
        self.latest_sent_velocity = [0.0] * 6
        ...
        with self.motion_lock:
            ...
            try:
                ...
                while self.velocity_servo_running.is_set() and not self.stop_event.is_set():
                    ...
                    self.latest_sent_velocity = vel6
