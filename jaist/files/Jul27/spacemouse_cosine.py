...
# 애매하게 비슷한 크기면(둘 다 못 이기면) 둘 다 죽임 -- 실수 입력 방지.
...
def command_from_state(state, seq, mode="digital"):
    ...
    # --- Yaw vs translation(x/y) 상호배타 처리 ---
    # 스페이스마우스를 비틀 때(yaw) 손목이 미세하게 x/y로도 힘을 줘서
    # 의도치 않은 병진 이동이 같이 발생하는 문제를 막기 위함.
    # raw 범위 대비 비율로 정규화해서 서로 다른 축끼리 크기를 비교 가능하게 함.
...
class PosePrinter:
    """
    Prints the current TCP [x, y, z] to the console at a genuinely fixed
    rate, using its own dedicated background thread with drift-corrected
    scheduling -- the same technique spacemouse_process() uses for its own
    60Hz read loop (accumulate next_tick += dt, sleep only the remainder).

    This is deliberately NOT tied to the main loop's timing: the main loop
    also does other work each tick (status draining, thread-health checks,
    optional --debug prints), and that variable amount of work was making
    the previous "print inside the main loop, gated by elapsed time"
    approach drift/jitter in practice. Running this in its own thread means
    the print interval stays consistent regardless of what else the main
    loop or velocity-servo thread are doing.

    No GUI, no cv2 -- console only.
    """

    def __init__(self, franka, hz):
        self.franka = franka
        self.dt = 1.0 / hz
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            if sleep_time > 0:
                ...
                # We fell behind (e.g. get_tcp_xyz took longer than dt) --
                # reset the schedule instead of trying to catch up with a
                # burst of back-to-back prints.
                next_tick = time.monotonic()
...
def main():
    ...
    parser.add_argument("--print-pose-hz", type=float, default=5.0,
                         help="Continuously print the current TCP [x,y,z] to the console at a "
                              "genuinely fixed rate (Hz), via a dedicated background thread with "
                              "drift-corrected scheduling -- independent of the main loop's own "
                              "timing, so the interval stays consistent even under load. "
                              "0 disables this (only the one-time startup pose is printed).")
    ...
    # Episode logging.
    parser.add_argument("--log-dir", default="episodes",
                         help="Directory to save per-episode CSV files into.")
    parser.add_argument("--log-hz", type=float, default=20.0,
                         help="Logging rate (Hz) while an episode is active.")
    parser.add_argument("--no-log", action="store_true",
                         help="Disable episode logging entirely (no 'e'-key window).")
    ...
    try:
        if not args.no_robot_motion:
            ...
            if args.print_pose_hz > 0:
                pose_printer = PosePrinter(franka, hz=args.print_pose_hz)
                pose_printer.start()

            if not args.no_log:
                # Action = latest_sent_velocity's [vx, vy, vz, wz] (indices
                # 0,1,2,5) -- the actual post-smoother command written to
                # the robot, not the raw pre-smoother spacemouse target.
                # This keeps state/action in sync despite the smoother's
                # transition delay (see earlier discussion on IL logging).
                def action_getter():
                    v = franka.latest_sent_velocity
                    return [v[0], v[1], v[2], v[5]]

                logger = EpisodeLogger(
                    state_getter=lambda: get_tcp_xyz(franka),
                    state_columns=["x", "y", "z"],
                    action_getter=action_getter,
                    action_columns=["vx", "vy", "vz", "wz"],
        ...
        if logger is not None:
            ...
            keypress.start()
            print("[Main] Episode logging ready (this terminal has focus):")
            print("[Main]   'e' -> start/stop episode (toggle)")
            print("[Main]   'q' -> quit")
        ...
        while True:
            if keypress is not None:
                ...
                # Periodic status line instead of a GUI panel -- cheap and
                # avoids any GUI toolkit dependency entirely.
                now_t = time.time()
                if now_t - last_status_print >= 2.0:
                    last_status_print = now_t
                    status_text = "RECORDING" if logger.is_active else "idle"
                    print(f"[Logger] {status_text} (episodes saved: {logger.episode_count})")
