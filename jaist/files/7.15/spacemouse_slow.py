...
Clean SpaceMouse teleop for Franka.

Architecture:
    SpaceMouse process:
        reads pyspacemouse
        maps raw state -> target velocity
        writes target velocity to shared memory

    Main process:
        creates FrankaController
        starts Franka velocity servo
        Franka servo reads shared command directly via velocity_getter
        main only monitors / stops

This removes:
    - main loop publishing velocity to Franka
    - Queue backlog
    - pub_hz timing problems
    - print/debug influencing command timing

Run:
    python3 spacemouse.py --ip 172.16.0.2

Run without robot:
    python3 spacemouse.py --no-robot-motion --debug
...
from rebuild.franka_slow import FrankaController
...
# Idle noise observed around 0.06, so 0.12 is safer.
DEADBAND_X = 0.12
DEADBAND_Y = 0.12
DEADBAND_YAW = 0.12
...
BUTTON_Z_SPEED = 0.004

# Yaw is the easiest to trigger reflex. Keep small.
MAX_WZ_POS = +0.020
MAX_WZ_NEG = -0.020
...
def command_from_state(state, seq: int) -> SPMCommand:
    now = time.time()
...
def zero_command(seq: int = 0) -> SPMCommand:
    return SPMCommand(
        stamp=time.time(),
        seq=seq,
    )
...
def write_shared(shared, cmd: SPMCommand):
    # No lock: one writer process, one reader thread.
    # Occasional partial read is acceptable because Franka loop smooths everything.
...
def read_shared(shared) -> SPMCommand:
    return SPMCommand(
...
def spacemouse_process(shared, connected_event, stop_event, status_q, read_count):
    """
    Separate process for SpaceMouse reading only.
    """
    try:
        # Lower priority so it does not disturb Franka communication.
        try:
            os.nice(10)
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument("--yaw-base", action="store_true")
    ...
    try:
        if not args.no_robot_motion:
            ...
            franka.start_ee_velocity_servo(
                ...
                max_linear_speed=0.010,
                max_yaw_speed=0.020,
                max_linear_acc=0.006,
                max_yaw_acc=0.015,
                target_lpf_alpha=0.10,
                command_timeout=0.40,
                startup_zero_time=1.0,
                max_distance_from_start=args.max_distance,
                yaw_in_base_frame=args.yaw_base,
        ...
        print("[Main] Franka thread reads SpaceMouse shared command directly.")
