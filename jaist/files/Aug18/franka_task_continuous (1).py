    ...
    def run(self):
        ...
        with pyspacemouse.open() as device:
            ...
            while self.shared.running:
                ...
                # CPU 양보 (busy-spin 방지)
                # =========================
                # device.read()가 non-blocking이라 sleep 없이 넘어가면 이
                # 루프가 CPU 코어 하나를 100% 점유하며 무한 반복하게 됨.
                # franky의 실시간 통신 스레드(1kHz)가 같은 코어에서 스케줄링을
                # 순간적으로 놓쳐 communication_constraints_violation이
                # 날 수 있어서, 1ms 정도의 짧은 sleep으로 CPU를 양보함.
                time.sleep(0.001)
                ...
                # 바로 CSV에도 기록 -- 조회/기록 주기를 분리해서 값이
                # 어긋나거나 초기화 전에 쓰이는 문제를 없앰.
                # act_v*는 franky의 실측 속도라 target과 달리 franky/Ruckig의
                # 저크 제한 때문에 약간의 지연을 보이는 게 정상.
